...
class LsdSwirl : Form
{
    ...
    {
        // Update 80x50 grid, map to 320x200
        for (int y = 0; y < 50; y++)
        {
            for (int x = 0; x < 80; x++)
            {
                ...
                // Map to 4x4 pixels (1x1 requested)
                for (int dy = 0; dy < 4; dy++)
                {
                    for (int dx = 0; dx < 4; dx++)
                    {
                        ...
                        {
                            vars[pixelY * 320 + pixelX] = colorIndex;
                        }
                    }
                }
        ...
        unsafe
        {
            ...
            for (int y = 0; y < 200; y++)
            {
                ...
                for (int x = 0; x < 320; x++)
                {
                    int index = vars[y * 320 + x];
                    int offset = x * 3;
                    row[offset + 0] = colorBytes[index, 0]; // B
                    row[offset + 1] = colorBytes[index, 1]; // G
                    row[offset + 2] = colorBytes[index, 2]; // R
                }
            }
        }
        ...
        byte pixel = vars[199 * 320 + 319]; // Use grid value
        bl ^= pixel;
        bl ^= var68c;
        bl ^= var68a;
        bl = (byte)(bl + var68d);
